using System.Text;
using CaptchaAI.Models.Responses;
using CaptchaAI.Tasks;
...
public class CaptchaAIClient
{
    ...
    private readonly static Uri _hostUrl = new Uri("https://api.captchaai.io");
    private readonly static Uri _betaHostUrl = new Uri("https://api-beta.captchaai.io");
    private readonly string _clientKey;
    private Proxy? _proxy = null;

    public CaptchaAIClient(string clientKey, bool beta = false)
    {
    ...
    public async Task<T> JoinTaskResult<T>(string taskId, int maximumTime = 120) where T : ITaskResponse
    {
        ...
        throw new CaptchaAIException(-1, "MAXIMUM_TIME_EXCEED", "Maximum time is exceed.");
    }
    ...
    private async Task<HttpResponseMessage> MakeRequest(string endpoint, string data)
    {
        ...
        {
            throw new CaptchaAIException(-1, "UNABLE_TO_MAKE_REQUEST", "Something is happened while making request");
        }
    ...
    private void CheckResponse(ErrorResponse? response)
    {
        if (response == null)
        {
            throw new CaptchaAIException(-1, "NO_RESPONSE", "The response is empty.");
        }
        else if (response.ErrorId != 0)
        {
            throw new CaptchaAIException(response.ErrorId, response.ErrorCode!, response.ErrorDescription!);
        }
    }
    ...
    private async Task<T> CheckResponse<T>(HttpResponseMessage response) where T : ErrorResponse
    {
        ...
        {
            throw new CaptchaAIException(-2, "NOT_VALID_RESPONSE", "The response is not valid.");
        }
    }
}
